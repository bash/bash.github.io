...
#   ╰────────────╮
#   ─────────────┴──────────────
=> (λm f.bind m (λx. Just (f x))) (Just X) double
#        ─┬──
#         │
#         ╰──────╮
#         ───────┴────────
=> (λm f.(λm f.m f Nothing) m (λx. Just (f x))) (Just X) double
=> (λm f.m f Nothing) (Just X) (λx. Just (double x))
=> (Just X) (λx. Just (double x)) Nothing
#   ─┬──         ─┬──             ───┬───
#    │            ╰─────────────╮    ╰──────────────────╮
#    ╰───╮                      │                       │
#   ─────┴──────           ─────┴─────               ───┴──
=> ((λa.λj n.j a) X) (λx. (λa.λj n.j a) (double x)) (λj n.n)
=> (λj n.j X) (λx. (λj n.j (double x))) (λj n.n)
=> (λx. (λj n.j (double x))) X
=> (λj n.j (double X))
```
...
#                     │
#   ──────────────────┴──────────────────
=> (λm p.bind m (λx.p x (Just x) Nothing)) (Just 3) isEven
=> bind (Just 3) (λx.isEven x (Just x) Nothing)
#  ─┬──
#   │
#   ╰──────╮
#          │
#   ───────┴────────
=> (λm f.m f Nothing) (Just 3) (λx.isEven x (Just x) Nothing)
=> (Just 3) (λx.isEven x (Just x) Nothing) Nothing
